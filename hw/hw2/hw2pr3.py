#!/usr/bin/env python3
"""
Sam Mikes smikes@g.hmc.edu

Start file for hw2pr3 of Big Data Summer 2020
...
def linreg(X, y, reg=0.0):
        ...
        # smikes note: I needed the solution to understand that 'do not regularize constant' meant i[0,0] = 0
        "*** END YOUR CODE HERE ***"
        return W_opt
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
        ...
        """
        # TODO: Generate a list of RMSE, RMSE_list
        RMSE_list = []
        reg_list = []
        W_list = []
        "*** YOUR CODE HERE ***"
        reg_list = sorted([150 * random.random() for _ in range(1,150)])

        for l in reg_list:
                W = linreg(X_train, y_train, reg=l)
                RMSE_list.append(find_RMSE(W, X_val, y_val))

        # smikes note: I had no clue what I was doing here
        "*** END YOUR CODE HERE ***"
        ...
        "*** YOUR CODE HERE ***"
        min_index = np.where(RMSE_list == np.amin(RMSE_list))[0][0]
        print(min_index)
        reg_opt = reg_list[min_index]

        print("min index: ", min_index)
        print("lambda: ", reg_opt)
        print("RMSE: ", RMSE_list[min_index])

        # smikes note: numpy docs give example of how to find min
        "*** END YOUR CODE HERE ***"
        return reg_opt
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
        ...
        # smikes note: my attempt (commented, below) did not give the
        # same results as numpy.norm
        # used algorithm from solution to make progress
        
#        for l in reg_list:
#                W = linreg(X_train, y_train, reg=l)
#                norm =  math.sqrt(np.sum([w*w for w in W])/m)
#                norm2 = np.linalg.norm(W,2)
#                print(norm,norm2)
